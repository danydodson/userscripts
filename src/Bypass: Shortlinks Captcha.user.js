...
// Change Your Delay in the settings options from 5 to 10 or 20 if you have issues like Your action marked Suspicious,Don't try to bypass ,Don't use Speedster, etc
   ...
   const cfg = new MonkeyConfig({title: 'Bypass Version 90.6 Settings :', menuCommand: true,
...
'use strict';
  ...
  BypassedByBloggerPemula(/revcut.net|urlcut.pro|faho.us|bitad.org|cutlink.xyz|c2g.at/, function() {RemoveRef('vk.com');});
  ...
  const sl = (h => {
    switch (h.host) {case 'multiup.org': if (h.href.includes('/download/')) return h.href.replace('download/', 'en/mirror/'); break;
      ...
      case 'miner-sim.com': case 'networkhint.com': case 'hostratgeber.de':
      ...
      case 'infonerd.org': case 'writeprofit.org':
  ...
  function onHtmlLoaded() {
    ...
    if (List.includes(location.host)) {ClickIfExists('a.btn.btn-success.btn-lg.get-link', 3, 'setInterval');
        ...
        $.ajax({type: "POST", url: url, data: form.serialize(), beforeSend: function(xhr) {pesan.attr("disabled", "disabled");$('a.get-link').text('Bypassed by Bloggerpemula');
          ...
          success: function(result, status, xhr) {if (xhr.responseText.match('(insfly|Insfly|linksly|cosmic-matter).pw|(freecrypto|a-s-cracks|freeltc).top|mdiskshortner.link|(oscut|exashorts).fun|bigbtc.win|gainl.ink|earnify.pro|clockads.in|(promo-visits|rexlink).site')) {location.href = result.url;} else {location.href = respect + result.url;}}});});}
