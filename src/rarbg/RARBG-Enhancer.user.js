...
var meta = {
	rawmdb: function () {
		...
		// @match        *.rargb.to/*
		// @match        *.rarbg.how/*
		// @match        *www2.rarbggo.to/*
		// @match        *.rarbg.proxyninja.org
		// @match        *.rarbgproxy.to/*
		// @match        *.rarbgproxy.to/*
...
(function () {
	...
	GM_config.init({
		...
		fields: {
			...
			"block Games": { label: "block Games", default: true, type: "checkbox", title: "Games" },
				...
				if (isOnTop10page) {
					function removeTop100(catcodes, slash_category) {
						try {
							...
							a.remove()
							var h1Rss = document.querySelector('body > table > tbody > tr > td:nth-child(2) > div > table > tbody > tr:nth-child(2) > td > h1 > a[href$="/rssdd.php?categories=' + catcodes.join(";") + '"]').parentElement
	 ...
	 * @param {string} title -
	 * @param {(number|string)=2} colIndex - 
	 *  A number specifying the column index to be inserted after (to the right of) (starts from zero).
