...
/* Save important original functions to prevent them from being polluted by external scripts */
...
* @param fn - Required, callback for when the element exists
* @param shadowRoot - Optional, specifies the DOM element under a shadowRoot to monitor
...
function ready(selector, fn, shadowRoot) {
  ...
  function $ready(selector, fn) {
    // Storing selectors and callback functions
    listeners.push({
      selector: selector,
      fn: fn
    });
...
/**
 * Some web pages use attachShadow closed mode and need to be open to access the video tag, such as Baidu Cloud Drive.
 * Solution reference:
...
function hackAttachShadow() {
  ...
  try {
    ...
    window.Element.prototype.attachShadow = function () {
      ...
      /* Allow elements in shadowRoot to have access to shadowHost */
      shadowRoot._shadowHost = this;

      // Add addShadowRoot custom event below document
      ...
      if (isClosed) {
        /**
         * Sets shadowRoot via defineProperty. Returns null when getting the value.
         * Ensures that the external environment still sees the closed shadowRoot to prevent misjudgment or targeted detection.
         */
...
/*!
* @name original.js
* @description Stores some important native functions to prevent external contamination. This logic should be pre-placed whenever possible, otherwise the stored functions will be contaminated.
* @version 0.0.1
* @author xxxily
...
const original = {
  ...
  // To prevent this type of gameplay: https://juejin.cn/post/6865910564817010702
  Proxy,
...
/**
* Media tag detection, capable of detecting media elements with modified tag names such as video, audio, and other media elements.
...
const mediaCore = (function () {
  ...
  /**
  * Creates API functions for enhanced control based on an HTMLMediaElement instance, enabling enhanced features such as locking playback speed, pausing, and playing.
  * @param {*} mediaElement - Required, a specific instance of HTMLMediaElement, such as the video tag or new Audio() on a web page.
  ...
  */
  function createMediaPlusApi(mediaElement) {
    ...
    const mediaPlusBaseApi = {
      /**
      * Create a lock to prevent external logic from manipulating mediaElement-related properties or functions.
      * The lock logic here is simply for data state marking and switching. The specific lock functionality needs to be implemented in
      * proxyPrototypeMethod and hijackPrototypeProperty.
      */
      ...
      isLock(keyName) {
        ...
        if (info.unLockTime) {
          // Delay lock calculates whether it is still in the locked state based on the current time
      ...
      // Note: Calling get, set, and apply here is not restricted by locks
      get(keyName) {
      ...
      set(keyName, val) {
        if (originDescriptors[keyName] && originDescriptors[keyName].set && !originMethods[keyName] && typeof val !== 'undefined') {
          // original.console.log(`[mediaPlusApi][${keyName}] Perform native set operations`)
      ...
      apply(keyName) {
        if (originMethods[keyName] instanceof Function) {
          ...
          args.shift();
          // original.console.log(`[mediaPlusApi][${keyName}] Execute native apply operation`)
    ...
    /**
    * Expand the API list to implement pure API call effects of 'playbackRate', 'volume', 'currentTime', 'play', and 'pause'.
    * The specific available APIs are as follows:
    *
    ...
    */
    ...
    extApiKeys.forEach(key => {
      baseApiKeys.forEach(baseKey => {
        /* When the key corresponds to a function, there should be no get or set API, but an apply API. */
  ...
  /* The processing logic of the media object is detected, relying on the proxy of the media function */
  ...
  /* The proxy method play and pause method ensures that the playback can be paused and played correctly */
  function proxyPrototypeMethod(element, methodName) {
    ...
    element.prototype[methodName] = new original.Proxy(originFunc, {
      apply(target, ctx, args) {
        ...
        // original.console.log(`[mediaElementMethodProxy] After executing the agent${methodName}function`)
        ...
        // TODO Observe and judge the results of function execution

        return result;
      }
    });

    // Extending the HTMLMediaElement prototype chain is not recommended, as it can easily allow web pages to detect the presence of MediaCore enhancement logic.
...
function openInTab(url, opts, referer) {
  ...
  // Because Tampermonkey's menu registration and unregistration features have some difficult-to-troubleshoot bugs,
  // we've limited the frequency of opening openInTab to address the issue of repeatedly opening the same URL when
  // clicking the Tampermonkey menu. This solution only addresses the symptoms and will still leave many issues with
  // menu registration and unregistration. We recommend replacing the Chrome extension with the current script functionality.

  // Use GM_getValue/GM_setValue or sessionStorage to control the frequency of calls to the same URL
  const now = Date.now();
  const sessionKey = `h5player_openTab_${url}`;
  let lastOpenTime = 0;

  // Use GM_getValue first/GM_setValue
  if (window.GM_getValue && window.GM_setValue) {
    ...
    if (lastOpenTime && (now - lastOpenTime) < 1000) {
      // console.info('Blocked from opening the same URL repeatedly:', url)
      return;
    }
...
const configManager = new ConfigManager({
  ...
  config: {
    ...
    media: {
      ...
      /* The last set playback speed, default is 1.5 */
    ...
    hotkeys: [
      {
        ...
        /* To disable the shortcut key, set disabled to true */
        disabled: false
      },
      {
        desc: 'full screen',
      ...
      {
        desc: 'Reduce video size -0.05',
        key: 'shift+x',
        command: 'setScaleDown',
        args: -0.05
      },
      {
        desc: 'Enlarge video size +0.05',
        key: 'shift+c',
        command: 'setScaleUp',
        args: 0.05
      },
      {
        desc: 'Restore video size',
        key: 'shift+z',
        command: 'resetTransform'
      },
      {
        desc: 'Move video 10px right',
        key: 'shift+arrowright',
        command: 'setTranslateRight',
        args: 10
      },
      {
        desc: 'Move the image 10px to the left',
        key: 'shift+arrowleft',
        command: 'setTranslateLeft',
        args: -10
      },
      {
        desc: 'Move screen up 10px',
        key: 'shift+arrowup',
        command: 'setTranslateUp',
        args: 10
      },
      {
        desc: 'Move screen down 10px',
        key: 'shift+arrowdown',
        command: 'setTranslateDown',
        args: -10
      },
      {
        desc: 'Move forward 5 seconds',
        key: 'arrowright',
        command: 'setCurrentTimeUp',
        args: 5
      },
      {
        desc: 'Move back 5 seconds',
        key: 'arrowleft',
      ...
      {
        desc: 'Speed ​​up playback',
        key: 'c',
        command: 'setPlaybackRateUp',
        args: 0.1
      },
      {
        desc: 'Play back to normal speed',
        key: 'z',
      ...
      {
        desc: 'Set the playback speed to 2x',
        key: 'Digit2',
        command: 'setPlaybackRatePlus',
        args: 2
      },
      ...
      {
        desc: 'Set the playback speed to 3x',
        key: 'Digit3',
        command: 'setPlaybackRatePlus',
        args: 3
      },
      ...
      {
        desc: 'Increase blur by 1 px',
        key: 'K',
        command: 'setBlurUp'
      },
      {
        desc: 'Decrease blur by 1 px',
        key: 'J',
        command: 'setBlurDown'
      },
      {
    ...
    blacklist: {
      /**
      * URL blacklist, prohibiting the h5player script from running under these URLs
      * This is useful for pages that are difficult to troubleshoot or difficult to access, but where you don't want to disable the entire site.
      * For example: the homepage of Bilibili
      */
      urls: [
  ...
  function init(pageWindow) {
    ...
    pageWindow.saveH5PlayerConfig = function (editor) {
      try {
        ...
        alert(`The configuration format is abnormal and the save fails:${e}`);
      }
    };
  }
...
/**
* Task Control Center
* Used to configure tasks that cannot be handled universally. For example, different websites have different full-screen methods, so you must use the website's own full-screen logic to ensure that subtitles, bullet comments, etc. function properly.
**/
...
/**
* Extracts a value from an object that can be stored in GM_setValue.
* @param obj {objcet} - Required: The object data to be stored.
* @param deep {number} - Optional: If the object hierarchy is very deep, the recursive level must be limited. By default, the maximum level is 3.
* @returns {{}}
*/
...
const monkeyMsg = {
  /**
  * Sends a message. In addition to sending the normal message, it also adds various necessary information.
  * @param name {string} - Required. Specifies the field to send to. The same field must be received for correct monitoring.
  * @param data {Any} - Required. Specifies the data to send.
  * @param throttleInterval - Optional. Because inexplicable duplicate messages may occur, to eliminate the side effects of duplicate messages, a throttling logic is introduced. This limits messages to only one within a certain time interval, automatically discarding any extra messages. The default is 80ms.
  * @returns {Promise<void>}
  */
...
/**
* Disable locking the playbackRate
* Some players prevent playbackRate modifications
* Use the hackDefineProperty function to prevent playbackRate modifications
...
function hackDefineProperCore(target, key, option) {
  ...
  if (target instanceof HTMLVideoElement) {
    ...
    if (unLockProperties.includes(key)) {
      try {
        debug.log(`Forbidden to lock ${key}`);
        option.configurable = true;
        key = key + '_hack';
      } catch (e) {
...
`;
    ...
    const faq = [
      {
        title: 'Why is my learning progress unchanged when I take classes at double speed?',
        action: 'openDocsLink',
        lang: ['zh'],
      ...
      {
        title: 'Baidu Netdisk videos cannot be played at double speed',
        action: 'openDocsLink',
        lang: ['zh'],
      ...
      {
        title: 'The playback speed is laggy, there is no progress, and the audio and video are out of sync.',
        action: 'openDocsLink',
        lang: ['zh'],
    ...
    function convertDropdownMenuToTemplate(dropdownMenu, isRootMenu = true) {
      const menuItems = dropdownMenu.map(item => {
        ...
        if (item.subMenu) {
          ...
          return `
            <sl-menu-item class="h5p-menu-action" value="${id}" title="${desc || title}" data-action="${item.action || ''}" data-args='${JSON.stringify(item.args || null)}' data-url="${item.url || ''}">
            ${title}
            </sl-menu-item>
          `;
        }
    ...
    function createRecommendModTemplate(refDom) {
      ...
      /* Compatible with various possible language configuration writing */
      const curLang2 = curLang.replace('-', '');
      const curLang3 = curLang.replace('-', '_');
      const curLang4 = curLang.split('-')[0];

      /* Filter out the recommendationList that matches the current language based on the current language and the languages ​​configuration of the recommendationList */
    ...
    /**
     * Register the Recommend switching logic. Every 4 seconds, check which h5p-recommend-item currently has an active h5p-recommend-item, then switch the active h5p-recommend-item to the next element, repeating this process.
     * Stop switching when the mouse moves over the recommendWrap element, and resume switching when the mouse moves away.
     */
...
*/
    ...
    const h5playerUI = {
      ...
      popup(element, h5Player) {
        ...
        /**
        * Checks whether popup initialization is abnormal. YouTube uses custom-elements-es5-adapter.js, which can cause popup errors, so this check is required.
        * Example: https://www.youtube.com/watch?v=jsb-5H_hy0M
        * Example: https://www.youtube.com/watch?v=-2xb7rGCi2k
        */
...
/* Defines which media tags are supported */
...
*/
  proxyPlayerInstance(player) {
    ...
    if (!player._hangUp_) {
      player._hangUpInfo_ = {};
      /**
      * Suspends a player function call.
      * @param name {String} - Required. The name of the player method or property. This name is externally visible. This method or property must be proxied to suspend the call; otherwise, the call will be invalid.
      * @param timeout {Number} - Optional. Suspend duration. Defaults to 200ms.
      * @private
      */
  ...
  /**
  * Initializes the autoplay logic
  * Autoplay will only occur if the autoplay button selector is configured
  */
  ...
  setPlaybackRate: function (num, notips, duplicate, skipLock) {
    ...
    try {
      ...
      originalMethods.Object.defineProperty.call(Object, player, 'playbackRate', {
        ...
        get: function () {
          /**
          * On YouTube, if playbackRateDescriptor.get.apply(player, arguments) is returned, speed adjustment may fluctuate and be abnormal.
          * The cause is currently unknown, so we'll return curPlaybackRate first.
          */
...
*/
  ...
  bindFakeEvent() {
    ...
    /**
    * Enables throttling to reduce the frequency of keystrokes.
    * Note that enabling throttling will prevent compound keys (e.g., shift+s) from working.
    */
    ...
    monkeyMsg.on('globalKeydownEvent', async (name, oldVal, newVal, remote) => {
      if (remote) {
        if (isInCrossOriginFrame()) {
          /**
           * If both pages are on the same cross-domain restricted page and are visible, they are likely to be in the same tab, but not 100% certain.
           * If the tabId is the same, they are 100% certain to be in the same tab.
           */
          ...
          /* Control video playback in picture-in-picture across tabs */
  ...
  init: function (global) {
    ...
    if (TCC && TCC.doTask('disable') === true) {
      debug.info(`[TCC][disable][${location.host}] Running video detection logic on this website has been disabled. You can view the relevant configuration in the Task Configuration Center for details.`);
      return true;
    }
    ...
    if (configManager.get('enableHotkeys') !== false) {
      ...
      debug.warn('Shortcut key capabilities have been disabled');
    }
...
async function h5PlayerInit() {
  try {
    if (isCloudflareChallengePage()) {
      console.warn('Currently on the cloudflare human-machine verification page, pausing the h5player', location.href);
      return false;
    }
  } catch (e) {
    debug.error(e);
  ...
  if (isInBlackList) {
    console.warn(`[h5player][config][blacklist][${location.href}] \The current page has been added to the blacklist and the relevant logic of the h5player enhanced script will not be executed. If you need to enable it, please remove the corresponding address from the blacklist in the configuration.`);
  }
  ...
  if (!isEnabled || isInBlackList) {
    debug.warn(`[config][disable][${location.host}] The current website has disabled scripts. To enable scripts, please turn them on in the menu.`);
    return false;
  }
  ...
  try {
    ...
    if (isInCrossOriginFrame()) {
      debug.log('Currently in a cross-domain restricted iframe, some h5Player functions may not be able to be activated normally', window.location.href);
    }
  ...
  /**
  * Interact with the official website's remote assistant. Those with serious security or information purists can manually comment out the following code.
  * The following code will not affect the normal operation of the main functions.
  * Uncommenting the code and disabling the UI will have the same effect.
  */
  try {
