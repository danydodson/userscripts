...
(function () {
	...
	const lang = navigator.userAgent == "Netscape" ? navigator.language : navigator.userLanguage
	let config = {}
	switch (lang) {
		case "zh-CN":
		case "zh-SG":
	...
	var _unsafeWindow = (typeof unsafeWindow == 'undefined') ? window : unsafeWindow//compatible ios userscripts Writing
	var storage = {
	...
	const configPage = ["https://github.com/hoothin/UserScripts/tree/master/Pagetual", "https://hoothin.github.io/UserScripts/Pagetual/"]
	...
	function geneSelector(ele, addID) {
		...
		//Google id Class is random.Baidu is more excessive, style script sequence is random
				...
				if (nextLink) {
					...
					while (parent && parent.tagName !== "BODY") {
						...
						if (parent.className && parent.classList) {
							if (parent.classList.contains("noClick") || parent.classList.contains("no-pages")) {
								this.nextLinkHref = false
								return null
							}
						}
	...
	function createPageBar(url) {
		...
		if (forceState == 2) {
			...
			inTable = example.parentNode.tagName == "TABLE" ||
				...
				(example.nextElementSibling && example.nextElementSibling.tagName == "TR") ||
				(example.nextElementSibling && example.nextElementSibling.tagName == "TBODY")
			inLi = example.tagName == "LI" || (example.nextElementSibling && example.nextElementSibling.tagName == "LI")
		}
