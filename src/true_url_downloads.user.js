...
(function () {
  'use strict';
  var shortcutKey = 88; // Shortcut Alt + x
  var altKey = true;
  var ctrlKey = false;
  var shiftKey = false;
  function Yu() {
    ...
    var linksArr = [];
    // Thunder, Express, Tornado URL decoding and decryption start here
    var Decryption = function (url) { // Decrypt first and then decode
      if (url != '') {
        var str = url.href,
          sr = url.href;
        var type = 0;
        ...
        if (sr != str) {
          url.title = '├This link has been decrypted┤';
    ...
    // Start searching for all link addresses
    function seekUrl() {
      var link;
      ...
      for (var i = 0, k = link.length; i < k; i++) {
        ...
        if (whref && /\w+href/i.test(lion)) { // Enumerate all dedicated chain properties
          if (whref) {
            link[i].href = whref;
    ...
    storage.getItem("dontDecode", v => {
      dontDecode = v ? true : false;
      if (dontDecode) return;
      setTimeout(() => {
        seekUrl();
    ...
    function copyLink() {
      showLinkFrame(function (r) {
        if (r) {
          _GM_setClipboard(r);
          _GM_notification("Copying successful!");
        } else {
          _GM_notification("There are no resources on the current page!");
        }
      });
    }
    _GM_registerMenuCommand("Toggle url decode on this page", () => {
      storage.setItem("dontDecode", !dontDecode);
      location.reload();
    });
