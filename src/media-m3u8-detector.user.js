...
(function () {
  ...
  if (location.host === "tools.thatwind.com" || location.host === "localhost:3000") {
    ...
    unsafeWindow.fetch = async function (...args) {
      try {
        ...
        // Failed request using proxy
        if (args.length == 1) {
          console.log(`Request agent:${args[0]}`);
  ...
  // Currently only used to obtain the top title
        ...
        showVideo({
          ...
          download() {
            const details = {
              ...
              headers: {
                // referer: location.origin, - this header is not allowed
...
(function () {
  ...
  function work() {
    ...
    for (let a of Array.from(document.querySelectorAll(
      ...
      if (a.nextSibling && a.nextSibling.hasAttribute && a.nextSibling.hasAttribute('data-wtmzjk-mag-url')) continue; // Already added
