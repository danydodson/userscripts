...
(async () => {
	...
	function getOpenAItoken() {
		return new Promise((resolve) => {
			...
			if (!accessToken) {
				GM.xmlHttpRequest({
					url: openAIendpoints.session, onload: (response) => {
	...
	function getAIGCFkey() {
		return new Promise((resolve) => {
			...
			if (!publicKey) {
				GM.xmlHttpRequest({
					method: 'GET', url: 'https://api.aigcfun.com/fc/key',
					headers: {
