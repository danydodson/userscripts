...
// @author            <china.qinwuyuan@gmail.com>
...
(function () {
  'use strict';
  //FIXME - Basic configuration
  var DEBUG = false; // Controls whether debug mode is enabled
  function DEBUG11() {
...
margin-bottom: 0;
  ...
  function shortLink() {
    ...
    if (current) {
      logMessage('shortLink', 'Remove existing short link element', false);
      // Remove the existing short link element
      current.remove();
    } else {
      logMessage('shortLink', 'Add short chain elements', true);
  ...
  //webhoot
  // STUB - Added opening settings to navigation bar
  if (setopenindoc) {
    addNavLink(translate('thisname'), '#', false, false, 'renminde');
    var customClassName = 'renminde'; // Custom class name
  ...
  // STUB - Comment area beautification options
  if (modifyRadioLabels) {
    if (document.location.pathname.endsWith('/feedback')) {
      ...
      // Iterate over the rating information object, obtain and process the raw text of each rating option
      ...
      // Update the rating options content on the page
    ...
    if (autocheck) {
      ...
      waitForElement('#enable-source-editor-code').then(() => {
        ...
        if (checkbox && !checkbox.checked) {
          ...
          logMessage('autocheck', 'Automatic click complete', true);
        }
      });
    }
  }
  ...
  function fetchHistoryVersions() {
    ...
    if (jsonUrl) {
      fetch(jsonUrl)
        ...
        .then(data => {
          logMessage('fetchHistoryVersions', 'Historical version', true, data);
...
`);
  ...
  //Obtained from the information page, if it is invalid in other such as code pages.
  ...
  function createScriptInfoHtml(scriptDetails, fname, fuRl, tran) {
    // Deconstruct the passed script details object
    const {
      url,
      name,
      description,
  ...
  function addScopedStyles() {
    GM_addStyle(/*css*/`
      ...
      /* Modal box title bar and bottom button style */
      ...
      #settingsModal .form-check-label {
        color: #000000;
        font-size: 16px; /* Here you can set the font size you want */
      }
      ...
      #settingsTabs {
        list-style-type: none; /* Remove the little black dots in front of list items */
      ...
      @media (max-width: 600px) { //NOTE - Fixed display issue on mobile devices
        #settingsModal {
          width: 100%;
          max-width: 100%;
          padding: 1rem;
      ...
      .nav-link {
        ...
        #settingsModal .nav {
        width: 30%; /* Reduce the width of the category switching bar */
      }
      #settingsModal .nav-link {
        font-size: 12px; /* Reduce font size */
      }
      #settingsModal .tab-content {
        width: 70%; /* Increase the width of the content area */
      }
    }
  `);
  }
...
`);
  ...
  const controls2 = [
    ...
    { type: 'link', id: 'collapseGreasyforkExpiredDiscussion', text: 'Fold greasyfork Expired discussion', href: 'https://greasyfork.org/scripts/426549/', target: '_blank' },
...
.code-container pre code .marker {
  ...
  function logUserScriptOccurrences(textContent) {
    ...
    if (textContent.includes(targetSubstring)) {
      ...
      if (urls.length > 0) {
        createDetailsWithLinks('#install-area', urls.map(url => [url, url]), '6666', `Quote${urls.length}files`);
      } else {
        logMessage('logUserScriptOccurrences', 'not found @require or @resource Related links', true);
      }
    } else {
      logMessage('logUserScriptOccurrences', 'The target file does not contain "==/UserScript==" substring', true);
    }
  }
  /**
* 从指定的 URL 中获取文本内容
...
*/
  ...
  function addNavLink(link_text, linkurl, newtab, lastone, lclassname) {
    ...
    if (nav) {
      ...
      logMessage('addNavLink', 'Navigation bar not found', false);
    }
  }
  //  -------------------------------------------------------------
  //--STUB - 增加本地中文汉化
  if (getCountryCode() === 'zh-CN') {
    ...
    translatet.set('Script Sets', 'Script Favorites');
    translatet.set('Edit', 'edit');
  ...
  async function autoLogin() {
    ...
    if (postResp.status !== 200) {
      ...
      logMessage('Automatic login', 'return code error', postResp.status, false);
      return;
    }
...
.lum-previous-button {
  ...
  /**
   * Beautify the top navigation bar
   */
...
///--STUB - Beautify web page badges, etc. greasyfork.org/scripts/436913
